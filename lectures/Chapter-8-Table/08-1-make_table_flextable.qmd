...
And the third form: a logical vector for rows, column numbers for columns. The logical vector has to be exactly as long as the table has rows, which is why the example builds ten TRUEs and FALSEs for a ten-row table. Get that length wrong and you get either an error or, worse, a recycled pattern selecting rows you did not intend. This form is most useful when you have computed the condition somewhere else and are carrying it around as a variable. If you are writing the condition inline, the formula version reads better.
...
This is the one that does what people expect autofit to do. You give it a maximum width in inches and it shrinks the columns until the whole table fits inside it. Same overlong country name as the previous tab, same autofit call, then a ceiling of six inches on top. The columns end up narrower and the long text wraps again, which is the trade you are making. Reach for this when a table is going into a document with a fixed page width and you would rather have wrapped text than a table running off the edge.

</details>

`fit_to_width()` shrinks the columns until the whole table fits inside the maximum width you give it (in inches).
