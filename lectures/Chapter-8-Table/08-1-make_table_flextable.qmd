...
And saving as an image, for when you need a picture rather than an editable table, typically for slides or a preprint. This route needs the webshot2 package, which drives a headless browser to take the screenshot. Note the install line has been updated: webshot2 is on CRAN now, so a plain install packages call is all you need rather than pulling it from GitHub. For pdf output, look at the zoom argument in the last example. Raising it increases the resolution, which matters if the image is going anywhere near print.
...
```{r eval = F}
#--- install ---#
#--- CHANGED: was remotes::install_github("rstudio/webshot2"). webshot2 has
#--- been on CRAN since 2021 (currently 0.1.2), so no GitHub install needed.
...
```{r eval = F}
save_as_image(table_temp, path = "temp_table.pdf", zoom = 10, webshot = "webshot2") 
```
:::
<!--end of panel-->
