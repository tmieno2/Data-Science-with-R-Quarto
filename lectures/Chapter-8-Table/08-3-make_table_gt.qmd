...
Totals and averages per row group, computed by gt rather than by you. It needs a stub to put the labels in, so the table has to have been built with rowname col, and it needs row groups for there to be groups to summarise. Fns takes a named list where the name becomes the label and the right-hand side is a formula, with the dot standing for the column being summarised. Fmt says how to display the results, and it is a separate argument because summary rows are not formatted by the fmt calls you applied to the body.

</details>

`summary_rows()` adds a summary row to each row group. The table needs a stub for the labels to sit in, and it needs row groups: called on a table with none, it is an error, and `grand_summary_rows()` is the function you want instead.
...
```{r eval = F}
gtsave(table_temp, "temp_table.pdf", zoom = 3)
```
