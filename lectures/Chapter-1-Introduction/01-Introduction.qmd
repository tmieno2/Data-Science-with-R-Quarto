...
Two navigation tricks before anything else, and since this is the first deck of the course they are worth a moment. The three stacked lines in the bottom left corner open a table of contents, so you can jump straight to a section. Pressing the letter o gives you a panel view of every slide at once. This deck is long, and it is the one you will come back to most often in the first few weeks, because it holds the basic vocabulary: what a vector is, what a data frame is, how to install a package. Being able to find one specific slide quickly matters more here than anywhere else.
...
The code boxes on these slides are live. R runs inside your browser, so you can try things out before you have R working on your own machine. The area with a hint of blue is an editor you can type in. Run Code executes everything in it, and you can run just part of it by highlighting that part and pressing Command Enter, or Control Enter on Windows. The two sheets icon in the top right copies the code so you can paste it into RStudio, and the reload button next to it puts the original code back when you have edited yourself into a mess. Do edit them. Changing a number and running it again is the fastest way to find out what something does, and nothing you type here can break anything.
