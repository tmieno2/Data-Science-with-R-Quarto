...
<span style = "color: red;"> Objective </span>: We would like to convert the wide data back to the original long data.
:::
<!--end of the 2nd column-->
...
+ <span style = "color: blue;">Rule 1</span>: It returns all rows from `data_x`, and all columns from `data_x` and `data_y`
+ <span style = "color: blue;">Rule 2</span>: Rows in `data_x` with no match in `data_y` will have NA values in the new columns 
+ <span style = "color: blue;">Rule 3</span>: If there are multiple matches between `data_x` and `data_y`, all combinations of the matches are returned
...
:::{.callout-note}
In this instance, <span style = "color: red;"> which comes first does not matter </span> because all the individual rows in `yield_data` (left data) have exactly one match in `price_data` (right data) without fail, and vice versa.
:::
