...
The usual two navigation tricks. The three stacked lines in the bottom left corner open a table of contents, and the letter o gives you a panel view of every slide. It is worth knowing your way around this deck, because it is really two topics: reshaping a single dataset between long and wide, and merging two datasets into one. When you are working on an assignment you will usually need one or the other rather than both, so jumping straight to the pivot slides or to the join slides is the point.
...
The code boxes are live. The faintly blue area is an editor, Run Code runs everything in it, and highlighting part of it and pressing Command Enter, or Control Enter on Windows, runs just that part. The copy button in the top right takes the code to your own machine, and the reload button restores the original after you have experimented. One thing specific to this deck: most cells run themselves as the slide loads, so you will often see the result before you touch anything. That is deliberate. With reshaping and merging, the lesson is what the table looks like afterwards, and comparing before with after is easier when the after is already on screen.
