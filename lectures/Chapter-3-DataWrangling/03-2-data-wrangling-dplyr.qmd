...
The tidyverse is not one package but a collection bundled together, and the list shows the members we care about: readr for reading files, dplyr for manipulating them, tidyr for reshaping, ggplot2 for plots, and stringr for text. Today is mostly dplyr. Two honest caveats. These are not the only way to do any of this; base R can do all of it, and data.table does much of it faster. We use the tidyverse because it is easy to learn and easy to read. And because it is enormously popular, help is easy to find, which matters more than elegance when you are stuck.
...
Selecting columns by number works, but I want to talk you out of it. The numbers carry no meaning: three and five tell a reader nothing about what those columns contain, including you in a month. Worse, they are fragile, because if the dataset gains a column at the front, every number shifts and your code quietly does something different. Referring to columns by name is better on both counts. Now look carefully at the two outputs on this slide, because they are not the same kind of thing, and the subsection underneath explains why that matters.
...
We also build a second, much smaller dataset called flights_mini, and the code that makes it uses several functions you have not met. Do not try to decode it now. The line underneath is the important instruction: you are not expected to understand this yet, and you will understand all of it by the end of the lecture. For the moment just run it and look at the result, so you know what columns it has and roughly what is in it, because most of the examples that follow use it.
...
Now the pipe, which changes how you write R more than any other single thing in this course. The rule is very simple: x pipe f is the same as f of x. The thing on the left becomes the input to the function on the right. Run both lines in the box and confirm they give the same answer. That is genuinely all the pipe does. It looks pointless on one function, and its value only becomes obvious when you chain four of them, which we get to shortly. The callout notes it comes from magrittr, which arrives with dplyr, so you never install it separately.
...
You just met the dot, which lets you send the piped object somewhere other than the first argument. The native pipe has the same idea, written as an underscore, but it is far more restricted, and this is where people get caught. Two restrictions. First, the underscore has to be attached to a named argument: you cannot just drop it in positionally the way you can with a dot. Second, it may appear only once in the call, so anything where you need the piped object in two places is simply out of reach. Both of those are errors, not silent misbehaviour, so at least you find out immediately.
...
+ `_` must attach to a **named** argument. `gsub(" ", "", _)` is an error.
...
rename does what its name says, and the only thing to get right is the direction, which people reliably get backwards. It is new name equals old name. The new one goes on the left, which matches how assignment works everywhere else in R, but reads oddly the first time because you are naming the thing that does not exist yet. The example puts the renamed and the original side by side so you can check the correspondence. When it goes wrong, it is almost always because the order was reversed.
...
The tabs here take it in two steps and then put them together, which is worth following carefully because the first step is confusing on its own. Grouping a dataset appears to do nothing: the printed output looks the same as before. All group_by has done is attach a note saying that subsequent operations should happen group by group, and the only visible sign is a Groups line in the printed header. The work happens on the second step, when summarize sees that note and produces one row per group instead of one row overall. The third tab writes the whole thing as a single pipe, which is how you will actually write it.
...
`flights_carrier` looks no different from `flights_slim`. All `group_by()` did was tell R that whatever you do next should be done group by group, which is why the printed output now carries a "Groups" line.
...
This is the single most common source of wrong dplyr answers, so please read it properly. When you group by two variables and then summarize, dplyr removes only the last grouping variable. The result is still grouped by the first one. It does tell you, in a message and in the Groups line of the output, but it is a message people learn to skim past. The consequence is that everything you do next silently happens by carrier rather than on the whole dataset, and your numbers are wrong in a way that looks perfectly plausible. Two fixes: pass dot groups equals drop to summarize, or call ungroup afterwards. And the habit in the callout: when a dplyr result looks strange, check the Groups line first.
...
Notice the `# Groups: carrier` line in the output. Anything you do next still happens **by carrier**, which is usually not what you meant.
...
This last section is labelled extensions, and the honest framing is that you do not need it today but you will be glad of it later. across lets you apply the same operation to many columns at once. It works inside summarize and mutate, and it exists because the alternative is writing out the same expression once per column, which is tedious to type and worse to read. If you ever find yourself copying and pasting a mean call four times and changing only the variable name, this is the tool you wanted.
...
The syntax has two parts: which columns to apply the function to, and which function to apply. The function is written in a slightly odd-looking way, as a backslash and then x in parentheses, which is R's shorthand for an anonymous function, meaning a function with no name defined on the spot. So backslash x, mean of x with na.rm true, means apply this particular version of mean. The reason for that wrapper is that it gives you somewhere to put the na.rm argument, which you almost always need with real data.
...
```{r}
#| eval: false
across(which variables to apply the function, function to apply)
```

The function is written as `\(x) f(x, options)`, an anonymous function:

```{r}
#| eval: false
across(c(arr_delay, dep_delay), \(x) mean(x, na.rm = TRUE))
```
<!-- CHANGED (A9): rewritten from `across(cols, mean, na.rm = TRUE)` to
     `across(cols, \(x) mean(x, na.rm = TRUE))`. Passing extra arguments through
     `...` has been deprecated since dplyr 1.1.0; verified by running the old
     form, which emits "The `...` argument of `across()` is deprecated". The old
     form still works today but warns, and students copy what they see. -->
...
flights %>% 
  ...
  summarize(
    across(
      c(arr_delay, dep_delay, air_time, time_hour),
      \(x) mean(x, na.rm = TRUE)
    )
  ) %>% head()
```

Note: `na.rm = TRUE` is an option for the `mean()` function, passed inside the anonymous function `\(x) mean(x, na.rm = TRUE)`.
...
This tab makes the case by showing both versions of the same thing. Without across, four nearly identical lines, differing only in a variable name, with all the copy-paste errors that invites. With across, one expression naming four columns. They produce the same answer. Beyond the typing saved, the second version states the intent more clearly: take the mean of these four columns, rather than here are four separate calculations that happen to be the same. That clarity is the real argument.
...
across becomes much more useful when combined with helper functions that select columns by pattern rather than by name. The list shows four: is.numeric and is.character select by column type, and starts_with and ends_with select by name pattern. The reason this matters is robustness. If you name four columns explicitly and someone adds a fifth, your code silently misses it. If you said all the numeric columns, it does not.

</details>

<br>

You can use `across()`, and other `dplyr` functions, together with helper functions that select variables matching a pattern you specify:
...
The first pattern is by type. Wrapping is.numeric inside where gives you every numeric column, so this computes the mean of all of them at once, whatever they happen to be called and however many there are. Note the where wrapper, because it is required and easy to forget: is.numeric is a test applied to a column, and where is what tells across to use it as a selector rather than as the function to apply.
...
The same idea for character columns, but note that the function applied is different, and sensibly so. Taking the mean of a text column is meaningless, so instead we use n_distinct, which counts how many unique values there are. That is genuinely informative: it tells you how many carriers, or how many airports, appear in each group. This is a good habit generally, asking how many distinct values a text column contains, because it tells you quickly whether something is a category or free text.
...
Now selection by name pattern. ends_with of delay picks up every column whose name ends in delay, which in this dataset gets you both arrival and departure delay without naming either. starts_with does the mirror image for prefixes. This is why consistent variable naming pays off: if all your outcome variables end in the same word, you can operate on the whole family in one line, and it keeps working when you add another one next month.
...
+ Use `starts_with()` to apply the same function to every variable beginning with a particular string of characters.
...
These selectors are ordinary expressions, so you can negate and combine them with logical operators. The exclamation mark in front of where is.numeric means everything that is not numeric. That is often the tidiest way to say what you want, because describing the small set and inverting it is easier than enumerating the large one. And a note in passing: this is the same logical vocabulary you learned in Chapter 1, being reused in a new place, which is a recurring pattern in R.
...
And finally you can mix approaches freely: name some columns explicitly, use a pattern for others, and use more than one across in a single summarize. The example does exactly that, taking the mean of month and all the delay columns, and counting distinct values in all the character columns, in one call. This is where across stops being a typing shortcut and becomes a way of saying precisely what you mean about a dataset whose exact shape you do not want to hard-code.

</details>

<br>

You can mix explicitly named variables with these helper functions:

```{webr-r}
flights %>% 
  group_by(carrier) %>% 
  summarize(
    across(c(month, ends_with("delay")), \(x) mean(x, na.rm = TRUE)),
...
One last point to close on, and it generalizes everything in the previous section. These pattern-based ways of referring to columns are not special to across; they work in the other dplyr functions too. So you can select all the numeric columns, or select everything ending in delay, or relocate all the character columns to the front, using exactly the same helpers. Try the three examples. Once you notice that the same vocabulary keeps working in new places, dplyr gets a great deal smaller to learn.

</details>

The same ways of referring to variables by pattern work in other `dplyr` functions too:
