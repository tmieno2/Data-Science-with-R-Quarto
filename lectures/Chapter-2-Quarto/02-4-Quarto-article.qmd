...
There is a real decision to make about which table package to learn, and it is worth a minute of thought. The most powerful ones, gt and flextable, can output for LaTeX, as can xtable, huxtable, and kableExtra. But look at the third bullet, because it gives you a reliable fallback when direct LaTeX output does not suit your document: gt and flextable can both export a table as an image, and LaTeX is perfectly happy to accept images. So there is no real penalty for investing your time in the more capable package.
...
Mathematical notation is where the output format you chose on the first slide starts to matter, which is exactly why this tab splits in two. If you are going to PDF through LaTeX, you can wrap your maths in an equals-tex block and write raw LaTeX, and because that passes straight through to LaTeX you can use any LaTeX maths tool you like. For maths that also works in HTML or Word, enclose the expression in double dollar signs instead. Both put the equation on its own line. The catch, in the second callout, is that the dollar-sign route does not support every LaTeX construct, so complicated expressions may not survive.
