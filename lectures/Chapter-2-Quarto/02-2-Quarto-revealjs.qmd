...
Change log at a glance:
  ...
  B. New material ......... presenting and navigating (keyboard keys), exporting
                            ...
                            filled in.
  C. Improvements ......... explained why the column div needs four colons,
                            separated fig- from out- properly, made the
                            scrollable demo actually overflow, chunk labels.

SAMPLE FILE: sample_qmd_revealjs.qmd is referenced but is not in the repository.
Either add it to this folder or drop the reference. Flagged inline below.
================================================================================
-->
...
Look at `sample_qmd_revealjs.qmd`.
:::
<!-- CHANGED (A7): quoted the callout title, which was "title=Direction". -->
<!-- NOTE (A/C): sample_qmd_revealjs.qmd is not in this repository. Either add
     it to this folder and link to it relatively, or remove this callout. -->
...
So you've rendered your slides and you've got an HTML file. Now what? You open it in a browser, and it behaves like any presentation tool you've used. The arrow keys or the space bar move you forward and back. Press f to go full screen so the browser chrome disappears. The two I really want you to remember are o, or escape, which zooms out to show every slide at once so you can click to jump anywhere, and m, which pulls up a menu listing all your slides by title. And if you forget all of this, just press the question-mark key during the talk and reveal.js shows you the complete list. So there's really nothing to memorize here.
...
<!-- ADDED (C1): entire tab. The original showed :::: on the outside and ::: on
     the inside without a word of explanation. This is the single most common
     thing students get wrong when they copy the block, and the error it
     produces (a mangled layout with no message) gives them no clue why. -->

<details class="transcript">
<summary>Transcript</summary>

Here's the subtlety. Look at the colons. The outer columns block uses four colons, and the inner column blocks use three. That is not a typo and it's not arbitrary. When you nest one of these fenced divs inside another, the outer one has to use more colons than the inner ones, because that's the only way Quarto can tell which closing line belongs to which block. The exact numbers don't matter — five outside and three inside works just as well — only that the outer count is larger. And here's why you should care, which is in the callout: if you use three for both, Quarto matches the wrong closing line, your columns silently collapse into a single column, and there is no error message telling you why. So when your side-by-side layout mysteriously renders as one column, count your colons first.

</details>

Notice the outer block uses **four** colons and the inner blocks use **three**.

+ When one of these blocks sits inside another, the outer one must be opened and closed with **more** colons than the inner ones.
+ That is how Quarto knows which closing line belongs to which block.
+ The exact number does not matter, only that the outer count is larger. Five outside and three inside works just as well.

:::{.callout-important}
If you use three colons for both, Quarto matches the first closing line with the outer block and your columns silently collapse into one. There is no error message, so this is worth remembering.
:::
...
A pre-made theme gets you close to what you want, but usually not exactly. To change the details, you write your own scss file and hand Quarto both — the default plus your custom file. The order matters here: default is applied first, then your custom scss goes on top, so anything you put in your own file wins. And this isn't hypothetical — every single lecture in this course is built exactly this way, with a file called custom.scss, and it's genuinely worth opening to see how it's done.

</details>

A pre-made theme gets you close. To change details, you write your own **scss** file and hand Quarto both:
...
+ `custom.scss` is applied second, so anything in it wins
...
+ Every slide title should now be red
:::
...
Here's a gotcha that bites people constantly. When you render my_talk.qmd, you get two things, not one: the HTML file, and a folder next to it, my_talk_files, that holds your figures, fonts, and all the reveal.js machinery. If you email just the HTML file — which feels like the natural thing to do — the person opens a broken page, because everything that made it work was in that folder you left behind. And this bites harder for slides than for a normal report, because the entire presentation framework lives in that folder.

</details>

Rendering `my_talk.qmd` produces **two** things:

+ `my_talk.html`
+ `my_talk_files/`, a folder holding the figures, fonts, and the revealjs machinery

Send only the html file and the recipient gets a broken page. This bites harder for slides than for a report, because the presentation framework itself lives in that folder.
...
As you saw in 02-1, the fix is one line: embed-resources set to true in your YAML header. That tells Quarto to pack everything, including figures, fonts, and the framework, into the single HTML file. Now you can email that one file, or upload it on its own, and it just works. The trade-off, in the callout, is that the file gets noticeably bigger and takes a moment longer to open. That's the price of it working anywhere, and it's almost always worth paying.
...
Everything is packed into the single html file, which you can then email or upload on its own.

:::{.callout-note}
The file gets noticeably larger, and self-contained slides take a moment longer to open. That is the price of it working anywhere.
:::
...
And here's how you make sure before you hit send. Render, then move only the HTML file — just that one file — to your Desktop, away from its folder. Open it there and click through a few slides. If the figures and the styling all survive the move, you know it's truly self-contained and safe to send. If they break, you forgot the embed-resources line. Thirty seconds of checking saves you an embarrassing follow-up email.
...
If the figures and the styling survive, it is safe to send.
