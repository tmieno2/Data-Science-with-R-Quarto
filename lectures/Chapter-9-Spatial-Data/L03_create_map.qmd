...
This slide is about where you put the data argument, and it is worth slowing down. Data named inside ggplot is global and every later geom inherits it. Data named inside a geom_sf belongs to that layer alone. The two cells look nearly identical and behave differently once you uncomment the extra layer: the first works because the global data is still available, the second fails because there is nothing for the new layer to draw. Read the second callout too, on why writing fill equals red inside aes gives you salmon and a stray legend rather than red polygons.
...
:::{.callout-important title="`aes(fill = \"red\")` does not fill red"}
Look at what the first one actually draws: salmon polygons and a legend with one
entry reading `red`. Anything inside `aes()` is treated as a **variable**, so
`fill = "red"` maps every polygon to the constant character value `"red"` and then
asks `ggplot2` to pick a colour for that category — which comes out `#F8766D`.

To actually set a colour, put it **outside** `aes()`: `geom_sf(fill = "red")`.
The same rule applies to `color`, `size` and `shape`. A stray legend appearing
where you expected none is the usual sign you have done this by accident.
:::
...
```{webr-r}
#--- CHANGED: aes(fill = NIR) removed. Naming a single layer and then         ---#
#--- faceting by layer produces exactly ONE panel, so the old code did not    ---#
#--- demonstrate faceting at all. Leave fill unmapped and you get one panel   ---#
