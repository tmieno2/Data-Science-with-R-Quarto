...
To reach the same five-row table with `extract()` and `group_by()`, map each
extracted `ID` to `target_rate`, then group by `target_rate`.

```{r eval = F}
#--- via extract ---#
terra::extract(NDRE, tb_prj) %>%
  dplyr::mutate(target_rate = tb_prj$target_rate[ID]) %>%
  dplyr::group_by(target_rate) %>%
  dplyr::summarize(NDRE = mean(NDRE, na.rm = TRUE))
```

<br>

:::{.callout-important title="They agree by default; `exact` changes the rule"}
Running both default routes gives the same five means, apart from floating-point
rounding, because they use the same cell-centre rule:

+ `rasterize()` assigns a whole cell to a polygon based on **where the cell centre
falls**. A cell half-covered is either in or out.
+ `extract()` also includes cells whose centres fall inside each polygon by default.
  With `exact = TRUE`, it includes every touched cell and reports its coverage
  fraction.
